...
const EditPurchaseOrder = () => {
  ...
  const { data: purchaseData, isLoading: purchaseLoading } = useTanstackQuery(
    `/purchase-order/${id}`
  );
  ...
  useEffect(() => {
    if (purchaseData && items) {
      const discountPercentage = (
        (purchaseData.discount_amount / purchaseData.total_amount) *
        100
      ).toFixed(2);
      const taxPercentage = (
        (purchaseData.tax_amount / purchaseData.total_amount) *
        100
      ).toFixed(2);

      setFormData({
        purchase_order_date: new Date(purchaseData.purchase_order_date),
        expected_delivery_date: new Date(purchaseData.expected_delivery_date),
        ...
        status_id: purchaseData.status_id,
        rows: purchaseData.purchase_order_details.map((detail) => {
      ...
      const initialTotals = {
        quantity: purchaseData.purchase_order_details.reduce(
  ...
  const onSubmit = async () => {
    try {
      ...
      const purchaseData = {
        ...
        purchase_order_date: formatDate(formData.purchase_order_date),
        ...
        purchase_order_details: formData.rows
      ...
      await axiosInstance.put(`/purchase-order/${id}`, purchaseData);

      await queryClient.invalidateQueries(["purchase-order"]);
  ...
  return (
    <div>
      <PageTitle title="Edit Purchase Order" />
          ...
          <div className="flex items-start gap-4">
            <div>
              ...
              <div className="w-auto mt-2">
                <DatePicker
                  date={formData.purchase_order_date}
                  setDate={(newDate) => {
                    setFormData((prev) => ({
                      ...prev,
